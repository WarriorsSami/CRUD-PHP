...
function createData () {
    ...
    $name = textboxValue ('team_name');

    if ($name) {
        $sql = 'INSERT INTO echipa (denumire) VALUES (?)';
...
function updateData () {
    ...
    if ($name) {
        $sql = "UPDATE echipa SET denumire=? WHERE id=?";
        $stmt = $conn->prepare ($sql);
...
function deleteData () {
    ...
    if ($result->num_rows > 0) {
        $sql = "DELETE FROM echipa WHERE id=?";
        $stmt = $conn->prepare ($sql);
        $stmt->bind_param ('i', $id);
